    ...
    def load_image(self, i: int, rect_mode: bool = True) -> tuple[np.ndarray, tuple[int, int], tuple[int, int]]:
        ...
        if im is None:  # not cached in RAM
            if fn.exists():  # load npy
                ...
                if str(f).lower().endswith(('.tif', '.tiff')):
                    import tifffile
                    # 1. 读取 4 通道物理光子 RAW 数据
                    im = tifffile.imread(f)

                    # 2. 尺度欺骗 (Scale Deception)
                    # 将 0~1 的物理 float32 放大到 0~255 的 float32。
                    # 原因是 YOLO 后续的 Mosaic、MixUp 和 cv2.warpAffine 等数据增强
                    # 默认图像是在 255 量级的。保持 float32 可以不丢失物理精度。
                    im = (im * 255.0).astype(np.float32)
                else:
                    im = imread(f, flags=self.cv2_flag)  # BGR
            ...
            if rect_mode:  # resize long side to imgsz while maintaining aspect ratio
                ...
                if r != 1:  # if sizes are not equal
                    ...
                    # cv2.resize 完全支持 float32 和多通道，所以这里不会报错
