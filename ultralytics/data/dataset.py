...
def verify_image_label_tiff(args):
    """
    自定义的图像标签验证函数，专门支持 4通道 Float32 TIFF。
    """
    # 🌟 修复核心：使用切片 [:7] 只取前7个参数，忽略后面多出来的参数 (如 rect 等)
    im_file, lb_file, prefix, keypoint, num_cls, nkpt, ndim = args[:7]
    ...
    try:
        ...
        if str(im_file).lower().endswith(('.tif', '.tiff')):
            try:
                import tifffile
                # 读取并获取形状 (Width, Height)
                tmp_img = tifffile.imread(im_file)
                # tifffile shape is (H, W, C), we need (W, H)
                shape = (tmp_img.shape[1], tmp_img.shape[0])
            except Exception as e:
                raise Exception(f"Corrupt TIFF file: {e}")
        else:
            # --- 普通图片走 PIL ---
