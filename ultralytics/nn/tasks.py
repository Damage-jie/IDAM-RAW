...
from ultralytics.nn.modules import DetISP, IlluminationDecouplingCore, RIDM
from ultralytics.nn.modules import AFMMFPN, AFMM
...
class BaseModel(torch.nn.Module):
    ...
    def loss(self, batch, preds=None):
        ...
        # 1. 计算 YOLO 原本的目标检测 Loss (Box, Cls, DFL)
        loss, loss_items = self.criterion(preds, batch)

        # ==========================================================
        # 🌟 引入 DetISP: Self-Boost Regularization 机制
        # ==========================================================
        try:
            # 获取网络的第一层，也就是我们刚刚写好的 DetISP
            m = self.model[0]

            if hasattr(m, 'lin_rgb_out') and hasattr(m, 'nonlin_rgb_out'):
                lin_feat = m.lin_rgb_out
                nonlin_feat = m.nonlin_rgb_out

                # 确保处于训练模式且特征已成功保存
                if lin_feat is not None and nonlin_feat is not None:
                    import torch.nn.functional as F

                    # 沿着 RGB 通道维度 (dim=1) 计算余弦相似度
                    # 迫使线性模块 (lin_feat) 的色彩方向对齐非线性模块 (nonlin_feat)
                    similarity = F.cosine_similarity(lin_feat, nonlin_feat, dim=1)

                    # 相似度越高(接近1)，Loss 越小
                    loss_self_boost = (1.0 - similarity).mean()

                    # 根据原论文公式(12)，设置 Self-Boost 的权重 lambda = 0.01
                    lambda_sb = 0.01
                    # lambda_sb = 0.00

                    # 将计算出的 Self-Boost 损失无缝叠加到 YOLO 的总损失上！
                    # 这样 PyTorch 在反向传播时，梯度会自动流回 DetISP 的特征层
                    loss = loss + lambda_sb * loss_self_boost

        except Exception as e:
            pass  # 兜底保护，如果跑其他纯 YOLO 模型会自动跳过，不报错
        # ==========================================================
...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        # 🌟 === 新增 AFMMFPN 动态导入保护 ===
        if isinstance(m, str) and m == "AFMMFPN":
            from ultralytics.nn.modules import AFMMFPN
            m = AFMMFPN
        else:
            m = (
        ...
        if m in base_modules:
            ...
            # 🌟 修复 1：兼容 f 为单个 int (-1) 或 list 的情况 (防止 AFMMFPN 报错)
            f_list = f if isinstance(f, list) else [f]
            in_ch = [ch[x] for x in f_list]

            # 🌟 修复 2：兼容 AFMMFPN 传出的嵌套特征列表，并自动解包
            if len(in_ch) == 1 and isinstance(in_ch[0], list):
                in_ch = in_ch[0]

            # 🚨 注意：这里只能有一句 extend！千万不要写两遍！
            args.extend([reg_max, end2end, in_ch])
        ...
        # === 新增 DetISP 解析逻辑 ===
            ...
            # c1 是输入通道 (4)，c2 是输出通道 (3, 因为要输出 RGB 给 YOLO 的 Backbone)
            c1, c2 = ch[f], 3
            args = [c1, *args]  # 将输入通道数传给 DetISP 的 __init__
        # 🌟 === 新增 IlluminationDecouplingCore 解析逻辑 === 🌟
        elif getattr(m, "__name__", "") == "IlluminationDecouplingCore":
            # IlluminationDecouplingCore(in_channels, kernel_nums)
            # args[0] 通常是 yaml 里传进来的 kernel_nums (例如 8)
            c1 = ch[f]  # 输入通道，通常是 3
            c2 = c1  # 输出通道保持不变，还是 3
            args = [c1, *args]  # 传给 __init__ 的第一个参数
        # 🌟 === 新增 AFMMFPN 解析逻辑 === 🌟
        elif getattr(m, "__name__", "") == "AFMMFPN":
            c1 = [ch[x] for x in f]  # 收集 Backbone 传来的 P3, P4, P5 多尺度通道
            c2_out = args[0]
            if c2_out != nc:
                c2_out = make_divisible(min(c2_out, max_channels) * width, 8)
            args = [c1, c2_out, *args[1:]]
            c2 = [c2_out, c2_out, c2_out]  # 极其关键：告诉下一层我们吐出了 3 个尺度！
        # ==================================
        # 🌟 === 新增 AFMM 解析逻辑 === 🌟
        elif getattr(m, "__name__", "") == "AFMM":
            c1, c2 = ch[f], args[0]
            if c2 != nc:
                c2 = make_divisible(min(c2, max_channels) * width, 8)
            args = [c1, c2, *args[1:]]
        # 🌟 === 新增 RIDM 解析逻辑 ===
        elif getattr(m, "__name__", "") == "RIDM":
            c1 = ch[f]
            c2 = c1  # 输入 3 通道，输出还是 3 通道
            args = [c1, *args]
